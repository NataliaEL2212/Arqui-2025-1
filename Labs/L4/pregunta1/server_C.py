...
#Servidor que espera 10 valores y obtiene la varianza muestral
...
if __name__ == '__main__':
    ...
    while True:
        ...
        try: 
            while True: 
                conn.sendall("FREE:C") #envia
                data = conn.recv(SOCK_BUFFER) 

                #Calcular varianza
                if(len(datos) == 10): #ya hay 10 datos

                    conn.sendall("BUSY:C") #envia
                    time.sleep(randint(1, 3)) #esperamos un tiempo aleatorio
                    datos = [int(val) for val in sockFile.split("\n")] #agrupamos los datos
                    varianza = statistics.pvariance(datos) #calculamos la varianza
                    nombre_C = f"C_metrics_{datetime.datetime.now()}.txt" #nombre del archivo B
                    with open(nombre_C, "w", encoding="utf-8"):
